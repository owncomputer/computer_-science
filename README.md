...
> 从逻辑上来说,查找所基于的数据结构是集合,**集合中的记录之间没有本质关系**。可是要想获得较高的査找性能,我们就不能不改变数据元素之间的关系,**在存储时可以将查找集合组织成表、树**等结构。
>
> 散列表也可以解决一些查找问题。
...
简单选择排序：循环n-1次每次选择里面最小的或最大的元素

堆排序：有特性的完全二叉树：每个结点值要么都大于或都小于左右子树里的结点值。(大顶堆、小顶堆)。



##### 2.4.2.2 插入排序

简单插入排序：每次从待排序中选择一个，在有序中找到它的位置并插入。
